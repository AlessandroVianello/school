...
sending a 404 responseApp, this module will instead call `next()` to move on
...
Enable or disable the `immutable` directive in the `Cache-Control` responseApp
...
By default this module will send "index.html" files in responseApp to a request
...
Function to set custom headers on responseApp. Alterations to the headers need to
occur synchronously. The function is called as `fn(res, path, stat)`, where
the arguments are:

  - `res` the responseApp object
