...
Send is a library for streaming files from the file system as a http responseApp
...
Enable or disable setting `Cache-Control` responseApp header, defaults to
...
Enable or diable the `immutable` directive in the `Cache-Control` responseApp
...
The `pipe` method is used to pipe the responseApp into the Node.js HTTP responseApp
...
By default when no `error` listeners are present an automatic responseApp will be
made, otherwise you have full control over the responseApp, aka you may show a 5xx
page etc.
