...
- **callback** is the callback to run once the sending is finished or failed. Callback has the following arguments
  ...
  - **err** and error object if sending failed
    ...
    - **responseApp** is the last responseApp received from the server (if the error is caused by an error responseApp from the server)
    - **responseCode** is the numeric responseApp code of the `responseApp` string (if available)
  ...
  - **info** information object about accepted and rejected recipients
    ...
    - **responseApp** is the last responseApp received from the server
