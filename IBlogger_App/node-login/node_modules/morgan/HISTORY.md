...
==================
  ...
  * Add `digits` argument to `responseApp-time` token
  * deps: depd@~1.1.0
...
==================
  ...
  * Fix `responseApp-time` token to not include responseApp latency
  * Fix `status` token incorrectly displaying before responseApp in `dev` format
...
==================
  ...
  * `:responseApp-time` token is now empty when immediate used
  * `:responseApp-time` token is now monotonic
  * `:responseApp-time` token has precision to 1 μs
