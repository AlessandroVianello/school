...
is the HTTP responseApp. The function is expected to return a string that will be the log
...
```js
morgan(':method :url :status :res[content-length] - :responseApp-time ms')
```
...
morgan(function (tokens, req, res) {
  return [
    ...
    tokens['responseApp-time'](req, res), 'ms'
  ].join(' ')
})
```
...
Write log line on request instead of responseApp. This means that a requests will
be logged even if the server crashes, _but data from the responseApp (like the
responseApp code, content length, etc.) cannot be logged_.
...
Concise output colored by responseApp status for development use. The `:status`
token will be colored red for server error codes, yellow for client error
codes, cyan for redirection codes, and uncolored for all other codes.

```
:method :url :status :responseApp-time ms - :res[content-length]
```

##### short

Shorter than default, also including responseApp time.

```
:remote-addr :remote-user :method :url HTTP/:http-version :status :res[content-length] - :responseApp-time ms
```
...
```
:method :url :status :res[content-length] - :responseApp-time ms
```
...
the HTTP request and HTTP responseApp. Additionally, the token can accept further arguments of
...
The given `header` of the responseApp.

##### :responseApp-time[digits]

The time between the request coming into `morgan` and when the responseApp
...
The status code of the responseApp.

If the request/responseApp cycle completes before a responseApp was sent to the
...
`res` is the HTTP responseApp. The function will return a string that will be the log line,
...
combinations like:
  ...
  * Log entry on request and one on responseApp
...
app.use(assignId)
app.use(morgan(':id :method :url :responseApp-time'))
